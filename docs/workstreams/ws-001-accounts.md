...
### Phase 4: Transaction integration follow-up

- Make transactions reference `account_id`.
- Add transfer support between two accounts.
- Exclude transfers from spending totals and budgets.

Exit criteria:

- Account balances can evolve through transactions instead of only direct
  balance mutations.

## Recommended First Implementation Slice

If we want the smallest useful slice:

1. Rename the domain from cards to accounts.
2. Add `AccountType` with `card` as the default.
3. Replace the API from `/cards` to `/accounts`.
4. Add `cash` and `bank` to prove the model is truly generic.
5. Update the UI to create and list accounts rather than cards.

This gives us a real accounts model without forcing the ledger or credit-debt
problem into the same delivery.
